...
const AdminComponent = () => {
    ...
    const inputIdFormNewsJSX = <input
        type="text"
        placeholder="id text"
    />
    const selectIdNewsImageJSX = <select ref={refIdImageNews} name="selectImage">
        <option hidden defaultValue>Id</option>
    </select>
    const inputImageNewsJSX = <><input ref={refNewsImage} type="file" name="imageNews" accept="image/*" /> <br /><br /></>
    ...
    const [formAboutImages, setFormAboutImages] = useState(<form encType="multipart/form-data">
        <input type="file" ref={refAboutImage} name="aboutImage" accept="image/*" /> <br /><br />
    </form>)
    const [idInputSelect, setIdInputSelect] = useState(<input ref={refIdTextAbout} type="text" placeholder="id text" />)
    const [inputContentTextAbout, setInputContentTextAbout] = useState(<><input ref={refAboutText} type="text" placeholder="content" /><br /><br /></>)
    ...
    const [inputCategorysPhoto, setInputCategorysPhoto] = useState(<><input ref={refFileCategorys} type="file" name="myImage" accept="image/*" /> <br /><br /></>)
    ...
    async function changeMethodPhotoProducts() {
        try {
            const val = refMethodImageProduct.current.options[refMethodImageProduct.current.selectedIndex].value;
            ...
            if (val) {
                if (val === "delete") {
                    ...
                    if (result !== null) {
                        const arr = [];
                        result.forEach((el, i) => {
                            const option = <option key={i} value={el}>{el}</option>;
                            arr.push(option);
                        })
                        setSelectIdImageProducts(<>
                            <select ref={refIdImageProducts}>
                                {arr}
                            </select>
                            <br />
                            <br />
                ...
                if (val === "post") {
                    setSelectIdImageProducts(<><input ref={refImageProducts} type="file" name="myImage" accept="image/*" /><br /><br /></>);
                    return;
                }
    ...
    async function clickImageProducts(e) {
        ...
        try {
            ...
            if (val) {
                ...
                if (val == "delete") {
                    const result = await getArrPhotoProducts();
                    const id = refIdImageProducts.current.options[refIdImageProducts.current.selectedIndex].value;
                    if (id) {
                        if (result !== null) {
                            ...
                            if (bool) {
                                ...
                                alert("Imaginea Nu Putut Fi Stearsa \n Dati Refresh La Pagina Si Incercati Din Nou ")
                                return;
                            }
                            alert("Nu Exista Asa Id")
                            return
                        }
                        alert("Ceva Probleme Cu Serverul Nu Sa Putut Sa Se Face Conexiunea Cu El ")
                        return
                    }
    ...
    async function changeMethodImageAbout(e) {
        try {
            ...
            if (val === "post") {
                setJsxFormImagesAbout(<form encType="multipart/form-data">
                    <input type="file" ref={refAboutImage} name="aboutImage" accept="image/*" /><br /><br />
                </form>);
                return;
            }
            if (val === "delete") {
                ...
                result.forEach((el, i) => {
                    arr.push(<option value={el} key={i} >{el}</option>)
                })
                setJsxFormImagesAbout(<><select onChange={(e) => setSelectImageName(e.target.value)}>
    ...
    async function changeIdProduct() {
        ...
        const val = refMethodProducts.current.options[refMethodProducts.current.selectedIndex].value;
        if (val === "put") {
            const id = refSelectProductId.current.options[refSelectProductId.current.selectedIndex].value;
            ...
            if (bool) {
                ...
                refFormProducts.current.inStock.options[refFormProducts.current.inStock.selectedIndex].value = obj.inStock;
                return;
            }
    ...
    async function changeMethodProduct() {
        const method = refMethodProducts.current.options[refMethodProducts.current.selectedIndex].value;
        if (method === "put") {
            const arrId = await getIdProducts();
            let arr = [];
            arrId.forEach((el, i) => {
                arr.push(<option key={i} value={el}>{el}</option>)
            })
            setFormProducts(<><form ref={refFormProducts}>
                <select
                    ref={refSelectProductId}
        ...
        if (method === "post") {
            setFormProducts(<><form ref={refFormProducts}>
                <><input type="text" name="id" placeholder="id text" /><br /><br /></>
                <input type="text" name="name" placeholder="text" /> <br />
                <input type="text" name="content" placeholder="continut" /><br />
                <input type="text" name="link" placeholder="link" /><br />
                <select name="categoryProduct" ref={refCategoryProduct}>
        ...
        try {
            ...
            if (confirm) {
                if (method === "post") {
                    ...
                    product.type = refCategoryProduct.current.options[refCategoryProduct.current.selectedIndex].value;
                    product.fotoPath = refImagePath.current.options[refImagePath.current.selectedIndex].value;
                    product.inStock = refFormProducts.current.inStock.options[refFormProducts.current.inStock.selectedIndex].value;
                ...
                if (method === "put" || method === "delete") {
                    product._id = refSelectProductId.current.options[refSelectProductId.current.selectedIndex].value;
                    ...
                    if (result !== null) {
                        ...
                        if (bool) {
                            ...
                            if (method === "delete") {
                                ...
                                alert("Ceva Probleme Cu Serverul Imaginea Nu A Putut Sa Fie Stearsa");
                                return
                            }
                            alert("Nu Exista Asa Metode")
                            return;
    ...
    async function clickCategorys() {
        const val = refCategorysMethod.current.options[refCategorysMethod.current.selectedIndex].value;
        if (val == "post") {
            const id = refIdCategory.current.value;
            const name = refNameCategory.current.value;
            const content = refCategoryAbout.current.value;
            const photoId = refCategoryPhoto.current.options[refCategoryPhoto.current.selectedIndex].value;
        ...
        if (val == "put") {
            ...
            const content = refCategoryAbout.current.value;
            const photoId = refCategoryPhoto.current.options[refCategoryPhoto.current.selectedIndex].value;
            const idSelect = refCategorysSelect.current.options[refCategorysSelect.current.selectedIndex].value;
    ...
    async function changeMethodCategorysPhoto() {
        try {
            const val = refMethodCategorysPhoto.current.options[refMethodCategorysPhoto.current.selectedIndex].value;
            if (val === "delete") {
                ...
                alert(" Ups anuntati programatorul care e responsabil pentru acesta eroare")
                setInputCategorysPhoto(<>Ups anuntati programatorul care e responsabil pentru aces eroare</>)
                return
            }
            if (val === "post") {
                setInputCategorysPhoto(<><input ref={refFileCategorys} type="file" name="CategorysPhoto" accept="image/*" /> <br /><br /></>);
                return
            }
        } catch (error) {
            console.log("error: ", error);
    ...
    async function clickCategorysPhoto(e) {
        ...
        try {
            const val = refMethodCategorysPhoto.current.options[refMethodCategorysPhoto.current.selectedIndex].value;
            if (val === "delete") {
                const id = refSelectCategoryPhoto.current.options[refSelectCategoryPhoto.current.selectedIndex].value;
                if (id) {
                    ...
                    return alert("Nu ati ales nici un id sau nu sunt asa id sau nu sunt fotografii ca sa stergi")
            ...
            if (val === "post") {
                ...
                if (file) {
                    ...
                    if (result)
                        alert("Imaginea a fost adaugata")
                    return;
                }
    ...
    async function changeMethodCategorys() {
        const method = refCategorysMethod.current.options[refCategorysMethod.current.selectedIndex].value;
        const result = await getArrImagesCategorys();
        if (!result) {
            return
        }
        ...
        setNameInputCategorys(<>
            <input ref={refNameCategory} type="text" placeholder="name" /><br />
            <input ref={refCategoryAbout} type="text" placeholder="despre categorie" /><br />
        ...
        if (method == "put") {
            ...
            arrCategorys.forEach((el, i) => {
                met.push(<option key={i} value={el._id}>{el._id}</option>)
            })
            setIsPutOrDelete(<select ref={refCategorysSelect}>{met}</select>)
        }
        if (method == "post") {
            setIsPutOrDelete(<input ref={refIdCategory} type="text" placeholder="id text" />)
        }
        if (method == "delete") {
            setNameInputCategorys("");
        }
    ...
    async function changeMethodAboutText(e) {
        setInputContentTextAbout(<><input ref={refAboutText} type="text" placeholder="content" /><br /><br /></>);
        const val = e.target.value;
        setMethodAboutText(val)
        if (val === "post") {
            console.log(val)
            setIdInputSelect(<input ref={refIdTextAbout} type="text" placeholder="id text" />)
        ...
        if (val === "put") {
            ...
            let arrOptions = [];
            result.forEach((el, i) => {
                arrOptions.push(<option value={el._id} key={i}>{el._id}</option>)
            })
            setIdInputSelect(<select onChange={() => {
                const id = refAboutId.current.options[refAboutId.current.selectedIndex].value;
        ...
        if (val === "delete") {
            ...
            setIdInputSelect(<select ref={refAboutId}>
                <option hidden>Id</option>
                {arrOptions}
            </select>)
        ...
        try {
            ...
            if (confirm) {
                if (methodAboutText === "post") {
                    if (refIdTextAbout.current.value.trim() === "" || refAboutText.current.value.trim() === "") {
    ...
    useEffect(() => {
        async function run() {
            ...
            let arrPhoto = [];
            result.forEach((el, i) => {
                arr.push(<option value={el._id} key={i}>{el.name}</option>)
            })
            resultPhotoArr.forEach((el, i) => {
                arrPhoto.push(<option value={el} key={i}>{el}</option>)
            })
    ...
    async function changeMethodNewsImageHandler() {
        try {
            const method = refMethodImageNews.current.options[refMethodImageNews.current.selectedIndex].value;
            if (method === "post") {
            ...
            if (method === "delete") {
                ...
                setSelectIdImageNews(<select ref={refIdImageNews}>
                    <option hidden defaultValue>Id</option>
                    {arr}
                </select>)
                return
            ...
            if (confirm) {
                ...
                if (method === "delete") {
                    ...
                    if (result && result.length > 0) {
                        const id = refIdImageNews.current.options[refIdImageNews.current.selectedIndex].value;
                        const bool = result.some(el => el === id);
                        if (!bool) {
    ...
    async function changeMethodNewsHandler(e) {
        ...
        if (method === "post") {
            ...
            arrImages.forEach((el, i) => {
                arrOptions.push(<option value={el} key={i} >{el}</option>)
            })
            setSelectImageFormNews(<><select>
        ...
        if (method === "put") {
            ...
            if (result) {
                ...
                const array = [];
                arrId.forEach((el, i) => {
                    array.push(<option value={el} key={i}>{el}</option>)
                })
                setSelectFormNews(<select>
                    <option hidden defaultValue>Id</option>
                    {array}
                </select>)
                return;
        ...
        if (method === "delete") {
            ...
            if (result) {
                ...
                setSelectFormNews(<select onClick={clickSelectFormNews}>
                    <option hidden defaultValue>Id</option>
                    {array}
                </select>)
                return;
    ...
    return (
        <>
            ...
            <div className="displayFlex">
                <div className={classes.width}><br />
                    <h2>Add New Product Photo</h2>
                    <div>Choose Image</div><br />
                    <select ref={refMethodImageProduct} name="imageProduct" onChange={changeMethodPhotoProducts}>
                    ...
                    <select ref={refMethodProducts} name="methodProducts" onChange={changeMethodProduct}>
                        <option hidden>Metoda</option>
                    ...
                    <div>
                        ...
                        {isPutOrDelete}
                        <select name="categorys" ref={refCategorysMethod} onChange={changeMethodCategorys}>
                            <option value="post">Post</option>
                            <option value="put">Put</option>
                            <option value="delete">Delete</option>
                        </select><br /><br />
                    ...
                    <div>
                        <select onChange={changeMethodCategorysPhoto} ref={refMethodCategorysPhoto}>
                            <option hidden>Metoda</option>
                    ...
                    <div>
                        ...
                        {jsxFormImagesAbout}
                        <button onClick={imageAboutClickHandler}>Post/Delete</button><br /><br /><hr />
                        <h2>Text About us</h2>
                        {idInputSelect}
                        ...
                        <button onClick={clickAboutTextHandler}>Add/Delete/Put</button><hr />
                    </div>
                    <div>
                        <h2>Image News </h2><br />
                        <form encType="multipart/form-data">
                            {selectIdImageNews}
                            <select ref={refMethodImageNews} onChange={changeMethodNewsImageHandler}>
                                <option hidden defaultValue>Method</option>
                                <option value="post">Post</option>
                                <option value="delete">Delete</option>
                            </select><br /><br />
                            {inputImageNews}
                            <button onClick={clickImageNewsHandler}>Add/Delete</button><hr />
                        </form>
                    </div>
                    <h2>News</h2>
                    <div>Create A New Post</div><br />
                    <form ref={refFormNews}>
                        ...
                        {selectImageFormNews}
                        {inputsFormNews}
                        <button onClick={clickNewsHandler}>Add/Delete/Put</button><hr />
                    </form>
                    ...
                    <button>Post/Delete</button><br /><br /><hr />
                    <h2>Servises Text</h2><br />
                    ...
                    <input type="text" placeholder="text" /><br />
                    <input type="text" placeholder="content" /><br /><br />
                    <button>Add/Delete/Put</button><hr />
                    <h2>Brands</h2>
                    <select name="selectImage">
                        <option value="asdasdasd">id_imagine</option>
                    </select>
