...
router.post("/categorysPhoto", uploadCategorys.single("CategorysPhoto"), async function (req, res) {
  try {
...
router.get("/products/:limit", async function (req, res) {
  try {
    ...
    if (limit && (limit === 12 || limit === 24 || limit === 48)) {
      res.setHeader('Content-Type', 'application/json');
      const products = req.app.locals.collectionProducts;
      const array = await products.find({}).limit(limit).toArray();
    ...
    res.status(500).send({ stat: null, message: "Ceva Probleme Cu Serverul Incercati Mai Tirziu" });
    console.log("error:", error);
  }
})
...
router.get("/threeProducts", async function (req, res) {
  try {
    ...
    if (type) {
      const threeProducts = await collectionProducts.find({ type: type }).limit(3).toArray()
...
router.post("/product", async function (req, res) {
  try {
    if (req.body) {
      ...
      res.status(500).send({ stat: null, message: "Datele Nu Au Putut fi Adugate " });
    }
    res.status(500).send({ stat: null, message: "Datele Nu Au Ajuns La Server " });
  } catch (error) {
    res.status(500).send({ stat: null, message: "Ceva Probleme Cu Serverul Incercati Mai Tirziu" });
    console.log("error: ", error)
  }
...
router.put("/products/:id", async function (req, res) {
  try {
    if (req.params.id && req.body) {
      const collectionProducts = req.app.locals.collectionProducts;
      const result = await collectionProducts.findOneAndUpdate({ '_id': req.params.id }, { $set: req.body })
      if (result) {
        res.setHeader('Content-Type', 'application/json');
        res.status(201).send({ stat: "ok", message: "The Product Changed" })
        return
      }
      res.status(500).send({ stat: null, message: "Datele Nu Au Putut fi Schimbate" });
      return
    }
    res.status(500).send({ stat: null, message: "Datele Nu Au Ajuns La Server Sau Nu Exista Asa Id " });
    return
  } catch (error) {
    console.log("error: ", error)
    res.status(500).send({ stat: null, message: "Ceva Probleme Cu Serverul Incercati Mai Tirziu" });
...
router.delete("/products/:id", async function (req, res) {
  try {
    if (req.params.id) {
      const collectionProducts = req.app.locals.collectionProducts;
      const result = await collectionProducts.findOneAndDelete({ '_id': req.params.id })
      if (result) {
        res.setHeader('Content-Type', 'application/json');
        res.status(201).send({ stat: "ok", message: "The Product Changed" })
        return
      }
      res.status(500).send({ stat: null, message: "Datele Nu Au Putut Fi Sterse" });
      return
    }
    res.status(500).send({ stat: null, message: "Datele Nu Au Ajuns La Server Sau Nu Exista Asa Id " });
    return;
...
router.get("/categorys", async function (req, res) {
  try {
    ...
    res.status(500).send({ stat: null, message: "Ceva Probleme Cu Baza De Date" });
  } catch (error) {
    console.log("error: ", error)
...
router.post("/categorys", async function (req, res) {
  try {
    if (req.body) {
      ...
      if (result) {
        res.status(200).send({ stat: "ok", message: "Categoria A Fost Adaugata " });
        return
      }
      res.status(500).send({ stat: null, message: "Ceva Probleme Cu Baza De Date" });
      return
    }
    res.status(500).send({ stat: null, message: "Datele Nu au Fost Transmise La Server" });
...
router.put("/categorys/:id", async function (req, res) {
  try {
    if (req.body && req.params.id) {
      const collectionCategorys = req.app.locals.collectionCategorys;
      const id = req.params.id;
      const result = await collectionCategorys.findOneAndUpdate({ _id: id }, { $set: { name: req.body.name } });
      if (result) {
        res.status(200).send({ stat: "ok", message: "Categoria A Fost Acutualizata " });
        return
      }
      res.status(500).send({ stat: null, message: "Ceva Probleme Cu Baza De Date" });
      return
...
router.delete("/categorys/:id", async function (req, res) {
  try {
    if (req.body && req.params.id) {
      ...
      if (result) {
        res.status(200).send({ stat: "ok", message: "Categoria A Fost Stearsa" });
        return
      }
      res.status(500).send({ stat: null, message: "Ceva Probleme Cu Baza De Date" });
      return
...
router.put("/about/:id", async function (req, res) {
  try {
    const collectionAboutUs = req.app.locals.collectionAboutUs;
    if (req.body) {
      const result = await collectionAboutUs.findOneAndUpdate({ '_id': req.params.id }, { $set: req.body });
      res.status(201).send("the text posted")
      return;
    }
...
router.delete("/about/:id", async function (req, res) {
  try {
    const collectionAboutUs = req.app.locals.collectionAboutUs;
    const result = await collectionAboutUs.findOneAndDelete({ '_id': req.params.id });
...
router.post("/imagesNews", uploadNews.single("imageNews"), async function (req, res) {
  try {
...
router.put("/news/:id", async function (req, res) {
  try {
    ...
    const collectionNews = req.app.locals.collectionNews;
    if (req.body && id) {
      const result = await collectionNews.findOneAndUpdate({ '_id': ObjectId(id) }, { $set: req.body })
...
router.get("/productsByCategorys/:id/:limit", async function (req, res) {
  try {
    ...
    const id = req.params.id;
    const limit = Number(req.params.limit);
    const result = await collectionProducts.find({ type: id }).limit(limit).toArray();
    res.status(201).send(result);
  } catch (error) {
    console.log("error:", error);
  }
